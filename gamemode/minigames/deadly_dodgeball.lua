MINIGAME.ID = 1; // Make sure this is a unique identifier. Used to synch client and server.
MINIGAME.Name = "Deadly Dodgeball"; // Used for display purposes only.
MINIGAME.Description = "Use the dodgeball to eliminate your opponents"; // The font used makes this all capped. No punctuation, please

MINIGAME.MinPlayers = 2; // Minimum players required to even be considered.
MINIGAME.MaxPlayers = MAX_PLAYER_COUNT_WHITEROOM_SQUARE; // Maximum players able to be supported.
...
function MINIGAME.Setup ( )	
	local DodgeballPos, DodgeballAng = GAMEMODE.ChooseRandomLocation('whiteroom_center_high');
	
	MINIGAME.Dodgeballs = {};
	
	local Dodgeball = ents.Create('ent_dodgeball');
	Dodgeball:SetPos(DodgeballPos);
	Dodgeball:SetAngles(DodgeballAng);
	Dodgeball:Spawn();
	
	local Phys = Dodgeball:GetPhysicsObject();
	if Phys and Phys:IsValid() then
		Phys:Sleep();
	end
	
	table.insert(MINIGAME.Dodgeballs, Dodgeball);
	
	MINIGAME.Go = CurTime() + 10;
end
...
function MINIGAME.PlayerLoadout ( Player )
	...
	return MINIGAME.Dodgeballs[1]:GetPos();
end
...
function MINIGAME.Start ( )
	MINIGAME.NextBallCreation = CurTime() + 15;
	MINIGAME.Dodgeballs[1]:GetPhysicsObject():Wake();
end

// Ran every tick while the minigame is being played.
function MINIGAME.Think ( )
	if MINIGAME.NextBallCreation and CurTime() > MINIGAME.NextBallCreation then
		local DodgeballPos, DodgeballAng = GAMEMODE.ChooseRandomLocation('whiteroom_center_high');
...
function MINIGAME.Cleanup ( )	
	if MINIGAME.Dodgeballs then
		for k, v in pairs(MINIGAME.Dodgeballs) do
			v:Remove();
		end
		
		MINIGAME.Dodgeballs = nil;
	end
	
	MINIGAME.NextBallCreation = nil;
end
